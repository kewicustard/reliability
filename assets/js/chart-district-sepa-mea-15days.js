...
function showGraphAndKpi_all(selectedYear, selectedDistrictValue, checkTarget) {
    ...
    if (typeof data == 'undefined' || data.data_year != selectedYear) {
        ...
        $.get("assets/php/all-district-sepa-mea-15days.php",
        { 
            selectedYear: selectedYear,
            selectedDistrictValue: selectedDistrictValue,
            checkTarget: checkTarget
        },
        function (res) {// get json data from all-district-sepa-mea-15days.php file
            console.log(res);
            window.data = res;
    ...
    function displayGraphandKpi_all(res) {// get json data from all-district-sepa-mea-15days.php file

        var saifi = [], saidi = [], saifi_target = [], saidi_target = [];
        var tabb = [], last_month = res.no_month, last_day = res.lasted_day;
        
        $("h2").html('District SEPA MEA Unofficial <span class="text-muted" id="selectedYear_head">(' + last_day + '/' + last_month + '/' + selectedYear + ')</span> (เทียบ<strong>ร่าง</strong>เป้าหมายปี 2563)');
