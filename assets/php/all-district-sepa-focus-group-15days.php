<?php
    ...
    // district code
    {
        $sql = 'SELECT code AS tabb_code, tabb FROM focusdist WHERE year = '.$selectedYear;
        try {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            $tabb_code[] = $row[tabb_code];
            $tabb[$row[tabb_code]] = $row[tabb];
        }
        // print_r($tabb_code);
        // print_r($tabb);
    }
    ...
    if ($target == 1) { // 1 has target for that year.
        ...
        $sql = 'SELECT month(yearmonthnumbertarget) AS data_month, districtCode, 
                ...
                FROM eachdisttarget_focus_group
                WHERE yearmonthnumbertarget BETWEEN "'.$selectedYear.'-01-01" AND "'.$selectedYear.'-12-01" AND districtCode in(SELECT code FROM focusdist WHERE year = '.$selectedYear.')';
    ...
    if ($lasted_day >= 28) {
        ...
        {
            $sql = 'SELECT district, month AS data_month, nocus AS dist_cust_month FROM discust WHERE district in(SELECT code FROM focusdist WHERE year = '.$selectedYear.') and year = '.$selectedYear.' and month <= '.$lasted_month;
            try {
                $stmt = $dbh->prepare($sql);
                $stmt->execute();
            } catch (PDOException $e) {
        ...
        {
            if ($lasted_month == 1) {
                $sql = 'SELECT district, month AS data_month, nocus AS dist_cust_month FROM discust WHERE district in(SELECT code FROM focusdist WHERE year = '.$selectedYear.') and year = '.($selectedYear-1).' and month = 12';
            } else {
                $sql = 'SELECT district, month AS data_month, nocus AS dist_cust_month FROM discust WHERE district in(SELECT code FROM focusdist WHERE year = '.$selectedYear.') and year = '.$selectedYear.' and month < '.$lasted_month;
            }
    ...
    // sum_cust_num and sum_cust_min
    {
        ...
        for ($i = 1; $i <= $no_district; $i++) { 
            for ($q = 1; $q <= $no_month; $q++) { 
                if ($row[$count_row][custdist] == $tabb_code[$i-1] && $row[$count_row][data_month] == $q) {
                    if ($row[$count_row][data_month] == 1) {
                        $eachCustNum[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_num];
                        $accuCustNum[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_num];

                        $eachCustMin[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_min];
                        $accuCustMin[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_min];
                    } else {
                        $eachCustNum[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_num];
                        $accuCustNum[$tabb_code[$i-1]][$q-1] = $accuCustNum[$tabb_code[$i-1]][$q-2]+$row[$count_row][sum_cust_num];
                        
                        $eachCustMin[$tabb_code[$i-1]][$q-1] = $row[$count_row][sum_cust_min];
                        $accuCustMin[$tabb_code[$i-1]][$q-1] = $accuCustMin[$tabb_code[$i-1]][$q-2]+$row[$count_row][sum_cust_min];
                    }
                    $count_row++;
                } else {
                        $eachCustNum[$tabb_code[$i-1]][$q-1] = 0;
                        $accuCustNum[$tabb_code[$i-1]][$q-1] = $accuCustNum[$tabb_code[$i-1]][$q-2]+0;

                        $eachCustMin[$tabb_code[$i-1]][$q-1] = 0;
                        $accuCustMin[$tabb_code[$i-1]][$q-1] = $accuCustMin[$tabb_code[$i-1]][$q-2]+0;
                }
            }
        }
        ...
        for ($i = 1; $i <= $no_district; $i++) { 
            if ($row[$count_row][custdist] == $tabb_code[$i-1] && $row[$count_row][data_month] == $no_month) {
                if ($row[$count_row][data_month] == 1) {
                    $eachCustNum[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_num];
                    $accuCustNum[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_num];

                    $eachCustMin[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_min];
                    $accuCustMin[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_min];
                } else {
                    $eachCustNum[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_num];
                    $accuCustNum[$tabb_code[$i-1]][$no_month-1] = $accuCustNum[$tabb_code[$i-1]][$no_month-2]+$row[$count_row][sum_cust_num];
                    
                    $eachCustMin[$tabb_code[$i-1]][$no_month-1] = $row[$count_row][sum_cust_min];
                    $accuCustMin[$tabb_code[$i-1]][$no_month-1] = $accuCustMin[$tabb_code[$i-1]][$no_month-2]+$row[$count_row][sum_cust_min];
                }
                $count_row++;
            } else {
                    $eachCustNum[$tabb_code[$i-1]][$no_month-1] = 0;
                    $accuCustNum[$tabb_code[$i-1]][$no_month-1] = $accuCustNum[$tabb_code[$i-1]][$no_month-2]+0;

                    $eachCustMin[$tabb_code[$i-1]][$no_month-1] = 0;
                    $accuCustMin[$tabb_code[$i-1]][$no_month-1] = $accuCustMin[$tabb_code[$i-1]][$no_month-2]+0;
            }
        }
